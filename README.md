...
```bash
modprobe tun && sysctl -w fs.inotify.max_user_watches=4194304 &&  sysctl -w fs.inotify.max_user_instances=8192 &&  sysctl -w fs.inotify.max_queued_events=65536 && sysctl fs.inotify &&  sysctl -w net.ipv4.neigh.default.gc_thresh1=1024 &&  sysctl -w net.ipv4.neigh.default.gc_thresh2=4096 &&  sysctl -w net.ipv4.neigh.default.gc_thresh3=8192 && apk update && apk add docker && rc-service docker start && rc-update add docker boot && docker --version && wget https://raw.githubusercontent.com/ngtanloc2410/Locne/refs/heads/main/Fast/deploy.sh && chmod +x deploy.sh && apk add unzip nano screen && wget https://github.com/ngtanloc2410/Locne/releases/download/tag/serverListTCP.zip && unzip serverListTCP.zip && clear
```
